      ...
      >
        ...
        >
          ...
          <base-video
              ...
              :is-shared="isShared || isVideoShared"
              ...
              :other-bottom-camera-ratio="otherBottomCameraRatio" :other-bottom-class-mode="otherBottomClassMode"
              otherCameraRatio
  ...
  >
    <div class="layout-container">
      ...
      <div
          ref="video-list-container"
          :class="{
            'video-list-container': !isShared && !isVideoShared,
            'video-list-container--with-share1': isShared || isVideoShared,
          }"
      >
        <div v-if="!isShared && !isVideoShared && !recordStatus" class="screen__recorder"></div>
        <ul
            ref="videoStreamContainer"
            :class="{ 'video-ul': true, 'video-ul--with-share': isShared || isVideoShared}"
        >
          <base-video
              v-for="(subscriber, index) in videoStreamsInLayout"
              :key="subscriber"
...
export default {
 ...
 async handleVideoFile(file){
   ...
   async subscribeVideoSharing(track){
    ...
    shareFullScreen() {
      if (this.$refs['video-share']){
        if (!this.isShareFullscreen){
          const videoShare = this.$refs["video-share"];
          ...
          this.isShareFullscreen = false;
        }

      }else {
        if (!this.isVideoShareFullscreen){
          ...
          this.isVideoShareFullscreen = true
        }else {
          if (document.exitFullscreen) {
            document.exitFullscreen();
          } else if (document.webkitExitFullscreen) {
          ...
          this.isVideoShareFullscreen = false;
        }
