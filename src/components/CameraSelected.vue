...
export default {
  ...
  mounted() {
    this.eventBus.on('confirm-choose-camera',()=> {
      if (this.camera.deviceId !== this.$store.getters.cameraId && this.stream){
        this.$store.dispatch('setCameraId', this.camera.deviceId)
        // this.$store.dispatch('setCamera', this.camera)
        this.eventBus.emit('camera_changed', this.camera.deviceId)
      }
    })
