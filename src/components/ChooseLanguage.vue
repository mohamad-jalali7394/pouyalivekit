<template>
  <div
      :style="{'background-color' :optionsList ? 'rgb(255, 254, 254)' : '#F6F6F7','box-shadow':optionsList ?  '0 1rem 2rem rgba(0, 0, 0, 0.26)':'','border':borderColor ? '0.1rem solid #DA0202' :'' }"
      :class="side">
...
<path
    ...
    fill="black"/>
        ...
        <span v-if="modelValue.color" style="width: 6px;height: 6px;border-radius: 50%"
              :style="{'background-color' : modelValue.color,'position': 'absolute','right':'-0.9rem'}"></span>
        <span v-show="isGroup ? isFirstTime : true" class="selected-option__text"
              :style="{'color' : textColor || 'black' }">{{ selectedOptionText }}</span>
    ...
    >
      <ul
          ...
          v-if="optionsList"
          :style="{ maxHeight:(isMobile && (isChat)) ? '30rem' :   maxHeight }"
          tabindex="0"
          ref="ulOptions"
      >
        <li
        ...
        >
          <div :style="{ height: '3rem' }" style="text-align: right">
            <label :class="selectSide" :for="optionValue.label" style="direction: rtl">{{
                optionValue.label
              }} </label>
              <input :id="optionValue.label" :name="optionValue.label" :checked="optionValue.label === selectedOptionText" type="radio">
          </div>
        </li>
      </ul>
    </transition>
