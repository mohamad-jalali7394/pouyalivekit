<template>
  <li ref="videoContainer" @mouseover="showMaximizeIcon" @mouseleave="hideMaximizeIcon" :class="[{[videoStreamClass] : true}, {'video-is-visible' : videoIsReady}]" :style="{ height: videoHeight }" >
    ...
    <div class="video-container" >
      ...
      <video v-show="cameraIsEnabled && hasVideoTrack" class="video-stream" :style="{'object-fit':'cover' , 'border-radius': '1rem'}" autoplay ref="video" @contextmenu.prevent :class="[{'active-speaker--active' : (activeSpeakerName === fullName)}]" playsinline></video>
      <div v-if="!cameraIsEnabled || reconnecting || !hasVideoTrack" :class="[{'active-speaker--active' : (activeSpeakerName === fullName)}]"   style="background-color:black ;width: 100%;height: 100%;position: absolute;z-index: 1;border-radius: 1rem" class="first--participant__icon" >
        <img  src="/img/avatar.svg" alt="not loaded" class="first--participant__icon">
      </div>
...
export default {
  ...
  computed:{
    ...
    videoStreamClass(){
      ...
      else if(this.index === 0 && this.layoutType === 'filmStrip'){
        return this.pinMode
      }else if (this.numberOfVideos === 5 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index < 2){
          return this.mode
        }else {
          return this.otherBottomClassMode
        }

      }else if (this.numberOfVideos === 7 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index < 3){
          return this.mode
        }else {
          return this.otherBottomClassMode
      ...
      }else if (this.numberOfVideos === 9 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index < 4){
          return this.mode
        }else {
          return this.otherBottomClassMode
      ...
      }else if (this.numberOfVideos === 11 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index > 7){
          return this.mode
        }else {
          return this.otherBottomClassMode
      ...
      }else if (this.numberOfVideos === 13 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index < 10){
          return this.otherBottomClassMode
        }else {
          return this.mode
      ...
      }else if (this.numberOfVideos === 14 &&  this.layoutType === 'flat' && !this.isShared){
        if (this.index < 10){
          return this.otherBottomClassMode
    ...
    videoHeight(){
      ...
      if (this.numberOfVideos === 5 &&  this.layoutType === 'flat' && !this.isShared) {
        if (this.index < 2) {
          return this.cameraRatio
        } else {
          return this.otherBottomCameraRatio
        }
      }else if (this.numberOfVideos === 7 &&  this.layoutType === 'flat' && !this.isShared) {
        if (this.index < 3) {
          return this.cameraRatio
...
.tile-view__with--5__video-streams{
  width: 42%;
  margin: 0.2%;
}
...
.tile-view__with--11__video-streams{
  width: 23.95%;
  margin: 0.1%;
}
...
.tile-view__with--13__up-video-streams{
  width: 19.82%;
  margin:0.05%;
}
.tile-view__with--13__down-video-streams{
  width: 33.1%;
  margin: 0.05%;
}
...
.tile-view__with--14__down-video-streams{
  width: 24.8%;
  margin: 0.05%;

}
.tile-view__with--14__up-video-streams{
  width: 19.8%;
  margin:0.05%;
}
