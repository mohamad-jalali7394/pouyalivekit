  ...
  text-overflow: ellipsis;">
    ...
    <div class="conference-footer__control-meeting">
      ...
      <div
          v-if="!isMobile"
          class="control-icon-fragment"
          :style="{cursor: !isLayoutchanging ? 'pointer' : 'not-allowed'}"

          @click="controlLayoutHandler('changeLayout')"
...
<!--      </div>-->
      <div
          v-if="(amIModerator || isModeratorSubstituteActivated) && isMobile"
      ...
      <div
          v-else-if="(!amIModerator && !isModeratorSubstituteActivated) && isMobile"
    ...
    <div>
      <div
          ...
          :style="{backgroundColor: isMyVideoStreamDisplayed ? '#1B76FF' : 'white', cursor: !selfViewIsToggling ? 'pointer' : 'not-allowed' }"
      ...
      >
        ...
        <img
            ...
            style="width: 3rem"
            :style="{cursor: !selfViewIsToggling ? 'pointer' : 'not-allowed'}"
             @click="controlSelfViewHandler('selfView', 'on')"
        />
        <img
            v-show="isMyVideoStreamDisplayed"
            draggable="false"
            ...
            style="width: 3rem"
            :style="{cursor: !selfViewIsToggling ? 'pointer' : 'not-allowed'}"
            @click="controlSelfViewHandler('selfView', 'off')"
        />
      </div>
      <div
          v-if="
...
export default {
      ...
      handler(val) {
        if (val) {
          ...
          setTimeout(() => {
            ...
            this.moreDropDownItems.splice(0,1)
          }, 10)
  ...
  methods: {
    ...
    controlLayoutHandler(controllerType, status) {
    ...
    controlSelfViewHandler(sideBarType, status) {
      if (this.selfViewIsToggling){
        return
      }
      this.selfViewIsToggling = true
      setTimeout(()=>{
        this.selfViewIsToggling = false
      },1000)
      this.$emit("control-side-bar-handler", sideBarType, status);
    },
  ...
  mounted() {
    this.eventBus.on('change-maximize_active_speaker',()=>{
      const selectedIndex = this.moreDropDownItems.findIndex((el)=>{
        return el.label === this.$store.getters.t("maximize_active_speaker")
      })
      if (selectedIndex > -1){
        this.moreDropDownItems[selectedIndex].label = this.$store.getters.t("minimize_active_speaker");
      }
    })
    this.eventBus.on('change-minimize_active_speaker',()=>{
      const selectedIndex = this.moreDropDownItems.findIndex((el)=>{
        return el.label === this.$store.getters.t("minimize_active_speaker")
      })
      if (selectedIndex > -1){
        this.moreDropDownItems[selectedIndex].label = this.$store.getters.t("maximize_active_speaker");
      }
