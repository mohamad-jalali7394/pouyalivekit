...
export default {
  ...
  watch:{
    ...
    isMyVideoStreamDisplayed(){
        navigator.mediaDevices
            .getUserMedia({video :  { deviceId: {exact: this.$store.getters.cameraId} }})
            .then(stream => {
  ...
  methods:{
    ...
    async controlMeetingHandler(type,stat) {
      switch (type) {
        ...
        case "microphone":
          ...
          break;
        case "camera":
            this.handleCamera(this.cameraStatus)
   ...
   async enterToRoom() {
    ...
    handlePublishResponse: async function(res) {
      ...
      if (data.activeSpeakerLayoutActive) {
        ...
        this.eventBus.emit('change-maximize_active_speaker')
      }
    },
    ...
    closeCameraAPIHandler(connectionId, targetUserId, name) {
      ...
      this.restClientControlMeeting.closeCamera(data)
          .then(() => {
            //   this.$store.dispatch(
            //       'addAllConnection', data);
            // })
    ...
    openCameraAPIHandler(connectionId, targetUserId, name) {
      ...
      this.restClientControlMeeting.openCamera(data)
          .then(() => {
            //   this.$store.dispatch(
            //       'addAllConnection', data);
            // })
    ...
    handleTrackUnsubscribed(track, publication, participant) {
      ...
      //
      // }
      if (!this.remoteTracksMap.get(userId)?.audioTrack && !this.remoteTracksMap.get(userId)?.videoTrack){
        this.remoteTracksMap.delete(userId)
      }
      // setTimeout(()=>{
    ...
    listenToCustomLiveKitMessages(payload){
      ...
      switch (message.action) {
        ...
        case "maximizeActiveSpeakerView":
          this.isActiveSpeakerMaximized = true
          this.isModeratorMaximizeActiveSpeaker = true
          if (this.amIModerator || this.isModeratorSubstituteActivated){
            this.eventBus.emit('change-maximize_active_speaker')
          }
          break;
        case "minimizeActiveSpeakerView":
          ...
          if (this.amIModerator || this.isModeratorSubstituteActivated){
            this.eventBus.emit('change-minimize_active_speaker')
          }
          break;
      ...
      try {
        if (!hasPublishedVideo) {
          ...
          this.videoLocalTrack = await createLocalVideoTrack({
            deviceId: {exact: this.$store.getters.cameraId},
      ...
      try {
        if (!hasPublishedAudio) {
          this.audioLocalTrack = await createLocalAudioTrack({
            deviceId: {exact: this.$store.getters.microphoneId},
          });
    ...
    updateLayout() {
      ...
      if (!this.isActiveSpeakerMaximized){
        if (this.layoutType === 'flat'){
          ...
          if (parseInt(this.layoutWith) === this.layoutWith){
            this.layoutWith = result.layoutWith + 'px'
          }
          this.otherBottomClassMode =  result.otherBottomClassMode
          this.otherBottomCameraRatio =  result.otherBottomCameraRatio
        }else {
        ...
        nextTick(()=>{
          if (!this.isMobile){
            if(this.isShared || this.isVideoShared){
              const result = filmStripView(this.isShared || this.isVideoShared,this.screenHeight, this.screenWidth, this.sideBarIsVisible,this.videoSubscribers?.length, this.isMobile)
              this.pinClassMode = result.pinClassMode
              this.classMode = result.pinClassMode
              this.firstCameraRatio = result.firstCameraRatio
              this.cameraRatio = result.firstCameraRatio
              this.isSharedWithSidebar = result.isSharedWithSidebar
              this.layoutWith = result.layoutWith
              if (parseInt(this.layoutWith) === this.layoutWith){
                this.layoutWith = this.layoutWith + 'px'
              }
            }else {
              if (this.$refs.conference.offsetWidth > (16/9) * (this.screenHeight - 65)){
                // this.layoutWith  = (16/9) * (this.screenHeight - 65) + 'px'
                this.layoutWith = (16/9) * (this.screenHeight - 65) + 'px'
              }else {
                // this.$refs.layoutContainer.style.width = '100%'
                this.layoutWith = '100%'
              }
              this.pinClassMode = 'full-screen__activeSpeaker'
              this.classMode = 'full-screen__activeSpeaker'
              this.firstCameraRatio = this.screenHeight - 65 + 'px'
              this.cameraRatio = this.screenHeight - 65 + 'px'
            }
    ...
    async changeMicrophone(id) {
      // Cycle through available cameras
        ...
        if (this.remoteTracksMap.has(myUserId)){
          this.audioLocalTrack = await createLocalAudioTrack({
            deviceId: {exact: this.$store.getters.microphoneId}, // Use selected mic
          });
          if (!this.micStatus){
            try {
              await this.audioLocalTrack?.disable();
   ...
   async changeSelfView(){
     this.isMyVideoStreamDisplayed = !this.isMyVideoStreamDisplayed
     if (this.isMyVideoStreamDisplayed){
       navigator.mediaDevices
           .getUserMedia({video :  { deviceId: {exact: this.$store.getters.cameraId} }})
           .then(stream => {
             this.selfViewStream = stream
       ...
       if (this.$refs.myVideoStream) {
         this.$refs.myVideoStream.srcObject = null;
       }
     }
    },
    onDrag(e) {
    ...
    handleCamera(status){
      this.setCameraState(!status)
      this.room.localParticipant.setCameraEnabled(!status);
    },
  },
  created() {
    this.isIOS = this.iOS()
