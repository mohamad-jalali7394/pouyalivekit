...
export function tileView(isShared, screenHeight, width, sideBarIsVisible, videoSubscribersLength) {
    ...
    const layoutConfig = {
        ...
        layoutWith: "100%",
        otherBottomClassMode:'',
        otherBottomCameraRatio:'',
    };
    ...
    if (isShared) {
        ...
        if (isShared === true) {
            layoutConfig.classMode = "tile-view__with--share";
            layoutConfig.cameraRatio = 0.24 * (screenHeight - 60) + "px";
        }
        return layoutConfig;
    }
    ...
    switch (videoSubscribersLength) {
        ...
        case 5:
            if (screenHeight > screenWidth){
                ...
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.33);

            }else {
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.55);
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.43);

            }
            layoutConfig.classMode = getLayoutClassFor5Participants(screenHeight, screenWidth);
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor5Participants(screenHeight, screenWidth);
        ...
        case 7:
            if (screenHeight > screenWidth){
                ...
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.245);

            }else {
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.54);
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.44);
            }
            layoutConfig.classMode = getLayoutClassFor7To8Participants(screenHeight, screenWidth);
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor7Participants(screenHeight, screenWidth);
            break;
        case 8:
            if (screenHeight > screenWidth){
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.245);
        ...
        case 9:
            ...
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor9Participants(screenHeight, screenWidth);
        ...
        case 10:
            ...
            layoutConfig.classMode = getLayoutClassFor10Participants(screenHeight, screenWidth);
            break;
        case 11:
            if (screenHeight > screenWidth){
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.245);
                ...
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.37);
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.3);

            }
            layoutConfig.classMode = getLayoutClassFor11Participants(screenHeight, screenWidth);
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor11Participants(screenHeight, screenWidth);
        ...
        case 13:
            if (screenHeight > screenWidth){
                ...
                layoutConfig.cameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT, 0.37);
                layoutConfig.otherBottomCameraRatio = calculateCameraRatio(screenHeight, screenWidth, MEETING_CONTROLS_HEIGHT,0.31 );

            }
            layoutConfig.classMode = getLayoutClassFor13Participants(screenHeight, screenWidth);
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor13Participants(screenHeight, screenWidth);
        ...
        case 14:
            ...
            layoutConfig.classMode = getLayoutClassFor14Participants(screenHeight, screenWidth);
            layoutConfig.otherBottomClassMode = getBottomLayoutClassFor14Participants(screenHeight, screenWidth);
...
function getLayoutClassFor5Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth){
      return   "tile-view__with--2__video-streams"
    }else if (screenHeight * 1.6 >= screenWidth){
      return   "tile-view__with--2__video-streams"
    }else{
      return   "tile-view__with--5__video-streams"
    }



}
function getBottomLayoutClassFor5Participants(screenHeight, screenWidth) {
    return screenHeight >= screenWidth
        ? "tile-view__with--2__video-streams"
        : "tile-view__with--5-6__video-streams";
}
...
function getBottomLayoutClassFor7Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.4) {
        return "tile-view__with--2__video-streams";
    } else if (screenHeight >= screenWidth) {
        return "tile-view__with--5-6__video-streams";
...
function getBottomLayoutClassFor9Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.4) {
        return "tile-view__with--2__video-streams";
    } else if (screenHeight >= screenWidth || screenHeight * 1.076 > screenWidth || screenHeight * 1.5 > screenWidth) {
        return "tile-view__with--5-6__video-streams";
...
function getLayoutClassFor10Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    } else if (screenHeight >= screenWidth * 1.4 || screenHeight >= screenWidth) {
        return "tile-view__with--5-6__video-streams";
...
function getLayoutClassFor11Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    } else if (screenHeight >= screenWidth * 1.4 || screenHeight >= screenWidth) {
        return "tile-view__with--5-6__video-streams";
...
function getBottomLayoutClassFor11Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    } else if (screenHeight >= screenWidth * 1.4 || screenHeight >= screenWidth) {
        return "tile-view__with--5-6__video-streams";
    } else if (screenHeight * 1.076 > screenWidth || screenHeight * 1.5 > screenWidth) {
        return "tile-view__with--11__video-streams";
    } else {
        return "tile-view__with--7-8__video-streams";
    }
}
...
function getLayoutClassFor13Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    }  else if (screenHeight * 1.076 > screenWidth) {
        return "tile-view__with--5-6__video-streams";
    } else {
        return "tile-view__with--13__down-video-streams";

    }
}
function getBottomLayoutClassFor13Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    }  else if (screenHeight * 1.076 > screenWidth) {
        return "tile-view__with--5-6__video-streams";
    } else {
        return "tile-view__with--13__up-video-streams";
    }
}



function getLayoutClassFor14Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    }  else if (screenHeight * 1.076 > screenWidth) {
        return "tile-view__with--5-6__video-streams";
    } else {
        return "tile-view__with--14__down-video-streams";

    }
}
function getBottomLayoutClassFor14Participants(screenHeight, screenWidth) {
    if (screenHeight >= screenWidth * 1.72) {
        return "tile-view__with--2__video-streams";
    }  else if (screenHeight * 1.076 > screenWidth) {
        return "tile-view__with--5-6__video-streams";
    } else {
        return "tile-view__with--14__up-video-streams";
    }
}
